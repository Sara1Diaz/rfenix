...
function JordanDamasj() {
    ...
    const fetchProductos = async () => {
        try {
            ...
            // Filtrar productos cuyo modelo empiece con "Jordan"
            const productosFiltrados = data.productos.filter(producto => producto.Modelo_producto.toLowerCase().startsWith('jordan'));
    ...
    const exportToExcel = () => {
        ...
        if (confirmExport) {
            ...
            saveAs(data, 'ProductosJordanDamas.xlsx');
        }
    };
    ...
    const exportToPDF = () => {
        ...
        if (confirmExport) {
            ...
            doc.text("Productos Jordan Damas", 14, 15);
            doc.autoTable({
                head: [tableColumn],
                body: tableRows,
                startY: 20,
            ...
            doc.save('ProductosJordanDamas.pdf');
        }
    };
